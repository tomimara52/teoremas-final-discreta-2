...
\label{sec:org932b21b}
Para esta demostración usaré las definiciones y resultados de \hyperref[sec:org3582695]{\uline{este teorema}}.
...
\label{sec:org3582695}
\subsection*{Definiciones}
\label{sec:org42fe9d6}
...
\uline{P}:
Cuando un vértice manda flujo a sus vecinos para balancearse, satura todos los lados por los que manda flujo, excepto quizás uno. Esto significa que para cada vértice en cada ola hacia adelante se satura parcialmente a lo sumo un lado. Entonces \(Compl(P) = O(n) \times \# \text{olas hacia adelante}.\)
...
\uline{Q}:
Cuando un vértice devuelve flujo a los vecinos, vacía todos los lados que devuelven flujo excepto quizás uno. Esto significa que para cada vértice en cada ola hacia atrás se vacía parcialmente a lo sumo un lado. Entonces \(Compl(Q) = O(n) \times \# \text{olas hacia atrás} = O(n) \times \# \text{olas hacia adelante} = O(n^2)\).
...
\label{sec:org87407e0}
Para ello probaré que 3SAT \(\le\) 3COLOR.

\subsection*{Algoritmo}
\label{sec:orgae0048d}
...
\label{sec:org91fed16}
\[\{(CAPI)e_{jr}\}_\substack{j=1,\cdots,m & r=1,2,3} \]

\item \underline{Lados}
\label{sec:orgca561d5}
\[ \{(CAPI)t\}\]

\item \underline{Lados}
\label{sec:orged8b68f}
...
\label{sec:org6ca1a3b}
\subsubsection*{\underline{\(\impliedby\) vuelta}}
\label{sec:orgdc15b52}
...
\label{sec:org44bbc0d}
\subsubsection*{\underline{\(\implies\) ida}}
\label{sec:orgfde982e}
